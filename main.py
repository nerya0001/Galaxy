...
class MainWidget(RelativeLayout):
    from transforms import transform, transform_2D, transporm_perspective
    ...
    def update_ship(self):
        ...
        x1, y1 = self.transform(*self.ship_coordinates[0])  # the * is expanding the tuple
        x2, y2 = self.transform(*self.ship_coordinates[1])
        x3, y3 = self.transform(*self.ship_coordinates[2])
        self.ship.points = [x1, y1, x2, y2, x3, y3]

    # check if the ship is still on the way
    def check_ship_collision(self):
        for i in range(0, len(self.ship_coordinates)):
            ti_x, ti_y = self.tiles_coordinates[i]
            # stop condition - there is no point in checking more then the bottom two rows
    ...
    # check if at least one point of the ship is still on the way
    ...
    def pre_fill_tiles_coordinate(self):
        # generate 10 tiles for the start
        for i in range(0, 10):
    ...
    def generate_tiles_coordinates(self):
        ...
        for i in range(len(self.tiles_coordinates), self.NB_TILES):
            ...
            # dealing with the tiles going out of the grid
    ...
    # keeps the vertical lines in place
    ...
    # keeps the horizontal lines in place
    ...
    # using the dt in order to get a consistent play speed no matter the machine
    def update(self, dt):
        time_factor = dt * 60
        ...
        if not self.state_game_over and self.state_game_has_started:
            ...
            spacing_y = self.H_LINES_SPACING * self.height
            # while and not if because in the start the dt is very big and the game is over
